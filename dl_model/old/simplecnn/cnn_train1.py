...
def train():
    ...
    base = Path(__file__).resolve().parents[1]  # 到 dl_model
    data_path = base / "mlp_feature_cache.jsonl"
    model_path = base / "simplecnn" / "best1.pth"
    ...
    # =========================
    # 🔴 保存 train/test audio_path 到 CSV
    # =========================
    train_csv = base / "train_paths.csv"
    test_csv = base / "test_paths.csv"

    with open(train_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["audio_path"])
        for r in train_r:
            writer.writerow([r.get("audio_path", "")])
